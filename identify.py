...
def train():
	print("Training the person dataset")

	recog = cv2.face.LBPHFaceRecognizer_create()

	dataset = 'persons'

	paths = [os.path.join(dataset, im) for im in os.listdir(dataset) if im.lower().endswith('.jpg')]
...
def identify():
	...
	paths = [os.path.join("persons", im) for im in os.listdir("persons") if im.lower().endswith('.jpg')]
	labelslist = {}
	for path in paths:
